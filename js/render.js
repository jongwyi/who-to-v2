...
export function renderProfileStep(step) {
    ...
    if (backBtn) backBtn.textContent = step === 1 ? '← Back' : '← Previous';
    const nextBtn = document.getElementById('btn-profile-next');
    if (nextBtn) nextBtn.textContent = step === 3 ? 'Submit ✓' : 'Next →';
    ...
    container.innerHTML = teams.map(team => {
        ...
        <div class="team-card">
            ...
            ${(team.members || []).map(member => {
                ...
                const roleLine = (first || second) ? `1st ${first ? first.emoji + ' ' + first.name : '—'} · 2nd ${second ? second.emoji + ' ' + second.name : '—'}` : '';
                ...
                return `<div class="member-card ${isNew ? 'member-card-new' : ''}">
                    <div class="member-name">${member.emoji || '👤'} ${member.name}${isNew ? '<span class="member-new-badge">new</span>' : ''}${state.currentStudent && member.id === state.currentStudent.id ? '<span class="member-you">(You)</span>' : ''}</div>
...
export function renderDashboard() {
    ...
    if (detailsEl) detailsEl.textContent = `Team Size: ${session.teamSize} • Weights: ${session.weightRole}% Role / ${session.weightInterest}% Interest`;
    const statusEl = document.getElementById('dashboard-status');
    if (statusEl) statusEl.textContent = session.status === 'published' ? '✅ Teams Published' : '⏳ Open for Registration';
    ...
    if (runBtn) runBtn.textContent = session.status === 'published' ? 'View Results →' : '🎲 Run Matching';
}
